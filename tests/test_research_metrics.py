...
def _levels():
    signal = TradeSignal(status=SignalStatus.VALID, setup_type=SetupType.OB_ENTRY, frame=Timeframe.M5, direction=Direction.BUY, entry_price=100.0, sl_price=99.0)
    return build_execution_levels(signal, min_rr=2.0)


def _bars():
    start = datetime(2026, 1, 1)
    rows = ((100, 100, 100, 100), (100, 102.1, 100, 102), (102, 104.1, 102, 104), (104, 104, 104, 104), (104, 104, 104, 104), (100, 103.1, 100, 103), (103, 103, 103, 103), (103, 103, 103, 103))
...
def test_metrics_are_derived_from_realized_trade_pnl():
    series = _bars()
    result = run_backtest(series, [BacktestOrder(series[0].timestamp, Direction.BUY, _levels()), BacktestOrder(series[4].timestamp, Direction.BUY, _levels())], costs=CostModel())
...
def test_metrics_preserve_cost_aware_net_pnl():
    series = _bars()
    result = run_backtest(series, [BacktestOrder(series[0].timestamp, Direction.BUY, _levels())], costs=CostModel(commission_per_volume=0.5))
    metrics = calculate_research_metrics(result)
...
def test_average_win_and_loss_are_arithmetic_means():
    start = datetime(2026, 1, 1)
    trades = (BacktestTrade(start, start, start, Direction.BUY, 100, 101, 1, 1, 0, 1, "TP"), BacktestTrade(start, start, start, Direction.BUY, 100, 104, 1, 4, 0, 4, "TP"), BacktestTrade(start, start, start, Direction.BUY, 100, 99, 1, -1, 0, -1, "SL"), BacktestTrade(start, start, start, Direction.BUY, 100, 98, 1, -2, 0, -2, "SL"))
    result = BacktestResult(trades, (10000.0, 10001.0, 10005.0, 10004.0, 10002.0), BacktestStats(10000.0, 10002.0, 2.0, 0.02, 4, 2, 2, 50.0, 1.6666666667, 0.5, 3.0, 0.03, 1.0))
    metrics = calculate_research_metrics(result)
...
def test_sharpe_is_not_falsely_annualized():
    start = datetime(2026, 1, 1)
...
def test_invalid_starting_equity_fails_closed():
    start = datetime(2026, 1, 1)
    trade = BacktestTrade(start, start, start, Direction.BUY, 100, 101, 1, 1, 0, 1, "TP")
    result = BacktestResult((trade,), (0.0, 1.0), BacktestStats(0.0, 1.0, 1.0, 0.0, 1, 1, 0, 1.0, float("inf"), 1.0, 0.0, 0.0, 0.0))
    with pytest.raises(ValueError, match="starting equity"):
        calculate_research_metrics(result)
