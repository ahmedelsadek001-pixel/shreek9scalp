...
def levels(direction=Direction.BUY):
    signal=TradeSignal(status=SignalStatus.VALID,setup_type=SetupType.OB_ENTRY,frame=Timeframe.M5,direction=direction,entry_price=100.0,sl_price=99.0 if direction==Direction.BUY else 101.0)
...
def test_spread_slippage_and_commission_reduce_pnl():
    ...
    result=run_backtest(series,[BacktestOrder(series[0].timestamp,Direction.BUY,levels())],costs=CostModel(slippage=0.1,point_value=1,commission_per_volume=0.5))
    assert result.trades[0].costs>0
    assert result.stats.net_pnl<3.0

def test_invalid_chronology_fails_closed():
    series=bars([(100,101,99,100,0),(100,101,99,100,0)]); series.reverse()
    with pytest.raises(ValueError,match="chronological"): run_backtest(series,[])
