...
def test_audit_chain_detects_reordering_or_tampering():
    first = _event()
    second = replace(
        _event(fingerprint_audit_event(first)),
        event_id="evt-2",
    )
...
def test_bridge_has_no_live_mode_and_enforces_account_expiry():
    ...
    assert request.is_acceptable(at=NOW, expected_account="acct-1")
    assert not request.is_acceptable(at=request.expires_at, expected_account="acct-1")
    assert not request.is_acceptable(at=NOW, expected_account="acct-2")
...
def test_bridge_rejects_malformed_signal_identity():
    ...
    assert not request.is_acceptable(at=NOW, expected_account="acct-1")
