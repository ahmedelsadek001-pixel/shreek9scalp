...
def find_all_order_blocks(
    ...
    """Find displacement-backed OBs using only candles already closed.

    The displacement look-ahead is bounded to the historical candles following
    the candidate candle; it is therefore valid for a signal evaluated only at
    the current closed-bar timestamp, but callers must not reuse a candidate
    before those confirmation candles exist.
    """
    ...
    avg_body = float(df["avg_body"].iloc[-1]) if "avg_body" in df.columns and pd.notna(df["avg_body"].iloc[-1]) else 0.0
...
def rank_order_blocks(
    ...
    for ob in obs:
        ...
        if direction == Direction.BUY and swing_low is not None:
            ...
            proximity = float("inf")
        ranked.append(OrderBlock(ob.high, ob.low, ob.bar_index, ob.body_size,
                                 ob.direction, dist, proximity))
...
def find_order_block(
    ...
    obs = find_all_order_blocks(df, direction, ob_min_body_atr, lookback)
    if not obs:
        return None
