...
def _bounded(df: pd.DataFrame, as_of_index: int | None) -> tuple[pd.DataFrame, int | None]:
    """Return the causal frame and its local final index."""
    ...
    if isinstance(as_of_index, bool) or not isinstance(as_of_index, int) or as_of_index < 0 or as_of_index >= len(df):
        raise ValueError("as_of_index must be a non-negative index within data")
    return df.iloc[:as_of_index + 1].copy(), as_of_index


def analyze_m15_entry_signal(df_m15: pd.DataFrame, trade_direction: Direction, settings: Settings, *, as_of_index: int | None = None) -> TradeSignal:
    frame = Timeframe.M15
    ...
    if trade_direction not in (Direction.BUY, Direction.SELL) or df_m15 is None or df_m15.empty:
        return empty
    try:
        df_m15, _ = _bounded(df_m15, as_of_index)
    except ValueError:
    ...
    if price is None or atr is None or atr <= 0:
        return empty
    confirmation = settings.swing_window.get("M15", 10)
    structure = determine_structure(df_m15, frame, atr=atr, threshold_atr=settings.bos_break_threshold_atr, confirmation_bars=confirmation, as_of_index=len(df_m15) - 1)
    bos_confirmed = _matching_event(structure, trade_direction)
    ob = find_order_block(df_m15, trade_direction, price, structure.last_swing_high, structure.last_swing_low, settings.ob_min_body_atr, as_of_index=len(df_m15) - 1)
    fvg = nearest_fvg(find_fvgs(df_m15, lookback=min(40, settings.fvg_max_age_bars), as_of_index=len(df_m15) - 1), price, trade_direction)
    sweep = detect_liquidity_sweep(df_m15, structure, as_of_index=len(df_m15) - 1)
    ...
    if combined: setup, confidence = SetupType.COMBINED, 0.95
    elif ob is not None and settings.m15_ob_entry: setup, confidence = SetupType.OB_ENTRY, 0.85
    elif fvg is not None and settings.m15_fvg_entry: setup, confidence = SetupType.FVG_ENTRY, 0.80
    elif sweep_confirmed: setup, confidence = SetupType.SWEEP_ENTRY, 0.70
    else: return TradeSignal(SignalStatus.NO_SIGNAL, SetupType.NONE, frame, trade_direction, price, details="No executable M15 setup", structure=structure)
    if candle_confirmed: confidence += 0.05
    if sweep_confirmed: confidence += 0.05
    if fvg_failure: setup, confidence = SetupType.FVG_FAILURE, min(0.99, confidence + 0.05)
    sl = _stop_from_zone(trade_direction, price, ob, fvg, structure, atr)
    if not isfinite(float(sl)): return TradeSignal(SignalStatus.NO_SIGNAL, SetupType.NONE, frame, trade_direction, price, details="Unable to construct protective stop", structure=structure)
...
def analyze_execution_frame(df: pd.DataFrame, trade_direction: Direction, frame: Timeframe, settings: Settings, *, as_of_index: int | None = None) -> TradeSignal:
    ...
    boundary = len(df) - 1
    structure = determine_structure(df, frame, atr=atr, threshold_atr=settings.bos_break_threshold_atr, confirmation_bars=settings.swing_window.get(frame.value, 12), as_of_index=boundary)
    ob = find_order_block(df, trade_direction, price, structure.last_swing_high, structure.last_swing_low, settings.ob_min_body_atr, as_of_index=boundary)
    fvg = nearest_fvg(find_fvgs(df, lookback=min(40, settings.fvg_max_age_bars), as_of_index=boundary), price, trade_direction)
    sweep = detect_liquidity_sweep(df, structure, as_of_index=boundary)
    sweep_confirmed = sweep.swept_low if trade_direction == Direction.BUY else sweep.swept_high
    ...
    combined = ob is not None and fvg is not None and not (float(ob.high) < float(fvg.bottom) or float(fvg.top) < float(ob.low))
    if failure and frame == Timeframe.M3: setup, confidence = SetupType.FVG_FAILURE, 0.95
    elif combined: setup, confidence = SetupType.COMBINED, 0.90
    elif ob is not None: setup, confidence = SetupType.OB_ENTRY, 0.85
    elif fvg is not None: setup, confidence = SetupType.FVG_ENTRY, 0.75
    elif sweep_confirmed: setup, confidence = SetupType.SWEEP_ENTRY, 0.65
    else: return TradeSignal(SignalStatus.NO_SIGNAL, SetupType.NONE, frame, trade_direction, price, details="No execution setup", structure=structure)
    if candle: confidence += 0.05
    if sweep_confirmed: confidence += 0.05
    sl = _stop_from_zone(trade_direction, price, ob, fvg, structure, atr)
    if not isfinite(float(sl)): return TradeSignal(SignalStatus.NO_SIGNAL, SetupType.NONE, frame, trade_direction, price, details="Unable to construct protective stop", structure=structure)
