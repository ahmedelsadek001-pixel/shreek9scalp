...
class CostModel:
    ...
    def valid(self) -> bool:
        ...
        return all(isfinite(float(x)) for x in values) and self.slippage >= 0 and self.point_value > 0 and self.commission_per_volume >= 0
...
def _pnl(entry: float, exit_price: float, direction: Direction, volume: float, point_value: float) -> float:
    return ((exit_price - entry) if direction == Direction.BUY else (entry - exit_price)) * volume * point_value


def _run_lifecycle(series: Sequence[BacktestBar], order: BacktestOrder, entry_index: int, entry: float,
                   costs: CostModel, policy: LifecyclePolicy,
                   trading_window: Optional[TradingWindowPolicy] = None) -> tuple[Optional[int], Optional[float], str, float, float, float, tuple[str, ...]]:
    ...
    stage_index = 0
    while last_index < len(series) and remaining > 0:
        ...
        if trading_window is not None and trading_window.holding_expired(series[entry_index].timestamp, bar.timestamp):
            exit_price = _exit_price(bar, order.direction, bar.open, costs.slippage)
            ...
            return last_index, exit_price, "TIME", gross_total, cost_total, remaining, tuple(events) + ("TIME",)

        stop = state.stop_price if state.stop_price is not None else order.levels.sl
        if state.trailing_active:
            from core.position_lifecycle import trailing_stop_price
            stop = trailing_stop_price(state, order.levels, bar.close, policy)
        ...
        if reason == "SL":
            gross_total += _pnl(entry, exit_price, order.direction, remaining, costs.point_value)
            return last_index, exit_price, "SL", gross_total, cost_total, remaining, tuple(events)
...
def run_backtest(bars: Iterable[BacktestBar], orders: Iterable[BacktestOrder], starting_equity: float = 10000.0,
                 costs: CostModel = CostModel(), force_close_at_end: bool = True,
                 lifecycle_policy: Optional[LifecyclePolicy] = None,
                 trading_window: Optional[TradingWindowPolicy] = None) -> BacktestResult:
    ...
    for order in pending:
        ...
        if lifecycle_policy is not None:
            exit_index, exit_price, reason, gross, cost, remaining, events = _run_lifecycle(
            ...
            if exit_index is None:
                ...
                gross = _pnl(entry, exit_price, order.direction, order.volume, costs.point_value)
