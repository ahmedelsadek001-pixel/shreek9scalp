...
This module calculates what the execution adapter *should* do after a fill.
It performs no broker I/O and cannot place or modify an order by itself.
"""
...
class LifecyclePolicy:
    ...
    def validate(self) -> None:
        ...
        if any(not isfinite(float(x)) or x < 0 for x in fractions):
            raise ValueError("TP fractions must be finite and non-negative")
...
def after_tp1(state: LifecycleState, levels: ExecutionLevels, policy: LifecyclePolicy, original_volume: float) -> LifecycleState:
    ...
    return LifecycleState(
        ...
        closed_tp1=True,
        closed_tp2=state.closed_tp2,
        closed_tp3=state.closed_tp3,
        breakeven_active=be,
        stop_price=stop,
...
def after_tp2(state: LifecycleState, original_volume: float, levels: ExecutionLevels, policy: LifecyclePolicy) -> LifecycleState:
    ...
    return LifecycleState(
        ...
        trailing_active=policy.trailing_after_tp2,
    )
