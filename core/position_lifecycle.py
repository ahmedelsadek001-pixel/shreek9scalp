...
class LifecyclePolicy:
    ...
    def validate(self) -> None:
        ...
        if any(not isfinite(float(x)) or x<0 for x in fractions): raise ValueError("TP fractions must be finite and non-negative")
        if abs(sum(fractions)-1.0)>1e-9: raise ValueError("TP fractions must sum to 1")
        if not isfinite(self.breakeven_offset) or self.breakeven_offset<0: raise ValueError("breakeven offset must be finite and non-negative")
        if not isfinite(self.trailing_distance_r) or self.trailing_distance_r<=0: raise ValueError("trailing distance must be positive")
...
def initial_state(volume:float, levels:ExecutionLevels, policy:LifecyclePolicy=LifecyclePolicy())->LifecycleState:
    policy.validate()
    if not isfinite(volume) or volume<=0: raise ValueError("volume must be positive and finite")
    if not isfinite(levels.risk) or levels.risk<=0: raise ValueError("execution levels must contain positive risk")
...
def _fraction_volume(remaining:float,original:float,fraction:float)->float:
    if not isfinite(original) or original<=0: raise ValueError("original volume must be positive and finite")
...
def tp1_action(state:LifecycleState,original_volume:float,levels:ExecutionLevels,policy:LifecyclePolicy)->LifecycleAction:
    policy.validate()
    if state.closed_tp1: raise ValueError("TP1 already processed")
    volume=_fraction_volume(state.remaining_volume,original_volume,policy.tp1_fraction)
    if volume<=0: raise ValueError("no volume available for TP1")
...
def after_tp1(state:LifecycleState,levels:ExecutionLevels,policy:LifecyclePolicy,original_volume:float)->LifecycleState:
    action=tp1_action(state,original_volume,levels,policy); stop=state.stop_price; be=state.breakeven_active
    if policy.breakeven_after_tp1:
        if levels.sl<levels.entry: stop=levels.entry+policy.breakeven_offset
        elif levels.sl>levels.entry: stop=levels.entry-policy.breakeven_offset
        else: raise ValueError("invalid execution levels: stop equals entry")
        be=True
...
def tp2_action(state:LifecycleState,original_volume:float,levels:ExecutionLevels,policy:LifecyclePolicy)->LifecycleAction:
    policy.validate()
    if not state.closed_tp1 or state.closed_tp2: raise ValueError("TP2 requires TP1 and must be processed once")
    volume=_fraction_volume(state.remaining_volume,original_volume,policy.tp2_fraction)
    if volume<=0: raise ValueError("no volume available for TP2")
...
def trailing_stop_price(state:LifecycleState,levels:ExecutionLevels,price:float,policy:LifecyclePolicy)->float:
    ...
    policy.validate()
    if not state.trailing_active: raise ValueError("trailing is not active")
...
def tp3_action(state:LifecycleState,levels:ExecutionLevels)->LifecycleAction:
    if not state.closed_tp2 or state.closed_tp3: raise ValueError("TP3 requires TP2 and must be processed once")
    if state.remaining_volume<=0: raise ValueError("no volume available for TP3")
